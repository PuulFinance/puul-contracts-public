...
import './SushiswapPoolHelper.sol';
import '../farm/IFarm.sol';
import '../protocols/sushiswap/SushiswapHelper.sol';

contract SushiswapPool is EarnPool, IPoolFarmExtended {
  ...
  SushiswapHelper private _helper;
  ...
  function setHelper(address helper) onlyAdmin external {
    ...
    _helper = SushiswapHelper(helper);
  }

  function _initialize() internal override returns(bool success) {
    _token0.safeApprove(address(UNI_ROUTER), 0);
    _token0.safeApprove(address(UNI_ROUTER), MAX_UINT);
    _token1.safeApprove(address(UNI_ROUTER), 0);
    _token1.safeApprove(address(UNI_ROUTER), MAX_UINT);
    return true;
  }
  ...
  function depositFromToken(address token, uint256 amountA, uint256 amountB, uint256 minSwapA, uint256 minSwapB, uint256 slippageA, uint256 slippageB, uint256 slippageRemA, uint256 slippageRemB) onlyMember nonReentrant external {
    uint256 liquidity = SushiswapPoolHelper.depositFromToken(address(_helper), token, amountA, amountB, _pair, minSwapA, minSwapB, slippageA, slippageB, slippageRemA, slippageRemB);
    _deposit(liquidity);
  }

  function depositPair(uint256 amount0, uint256 amount1, uint256 minA, uint256 minB) onlyMember nonReentrant external {
    uint256 liquidity = SushiswapPoolHelper.depositPair(amount0, amount1, _pair, minA, minB);
    _deposit(liquidity);
  }
  ...
  function depositSingleSided(address token, uint256 amountA, uint256 amountB, uint256 minSwapB, uint256 slippageA, uint256 slippageB, uint256 slippageRemA, uint256 slippageRemB) onlyMember nonReentrant external {
    uint256 liquidity = SushiswapPoolHelper.depositSingleSided(address(_helper), token, amountA, amountB, _pair, minSwapB, slippageA, slippageB, slippageRemA, slippageRemB);
    _deposit(liquidity);
  }

  function depositLPToken(uint256 amount) onlyMember nonReentrant external {
    uint256 liquidity = SushiswapPoolHelper.depositLPToken(_pair, amount);
    _deposit(liquidity);
  }
  ...
  function claimToToken(address token, uint[] memory amounts, uint[] memory mins) external override nonReentrant {
    ...
    SushiswapPoolHelper.claimToToken(_helper, token, amounts, mins, _owedRewards, _rewards, _rewardTotals);
  }
  ...
  function _removeLiquidity(IERC20 pair, uint256 amount, address to, uint256 minA, uint256 minB) internal virtual returns(uint256 token0, uint256 token1) {
    pair.safeApprove(address(UNI_ROUTER), 0);
    pair.safeApprove(address(UNI_ROUTER), amount * 2);
    (token0, token1) = UNI_ROUTER.removeLiquidity(address(_token0), address(_token1), amount, minA, minB, to, now.add(1800));
  }
