...
contract("Curve3Usdn", async accounts => {
  let instance;
  let helper;
  ...
  let lp = '0x4f3E8F405CF5aFC05D68142F3783bDfE13811522';
  ...
  async function initialize() {
    ...
    instance = await CurveUsdn.deployed();
    farm = await CurveUsdnFarm.deployed();
  ...
  it("fund CurveUsdn with USDT", async () => {
    ...
    console.log(`
      Deposit into the CurveUsdn using USDT.
    `);
   ...
   `);
    ...
    `);
    try {
      const bal3 = await logEtherBalance('usdn balance before deposit', LP, member1);
      await instance.depositLP(bal3, {from: member1});
    } catch (e) {
      console.log('depositLP', e);
      assert.equal(2, 1, 'depositLP failed');
    }
    await logEtherBalance('usdn balance after deposit', LP, member1);

    console.log(`
      Withdraw part into an usdn token.
    `);
    try {
      let slip = new BN('100');
      let bal = toWei('1000');
      let minw = await instance.estimateWithdrawUsd(usdn, bal, slip, { from: member1 });
      logEther('minw', minw);
      logEtherBalance('lp bef', USDN, instance.address);
        await logEtherBalance('usdn balance member1 bef', USDN, member1);
      await instance.withdrawUsd(usdn, bal, minw, { from: member1 });
    } catch (e) {
      console.log('withdrawUsd failed', e);
    }
    let usdnbal = await logEtherBalance('usdn balance after withdrawal member1', USDN, member1);

    console.log(`
      Deposit the usdn tokens back.
    `);
    try {
      const slip = new BN('100');
      const min = await instance.getMinAmount(usdn, usdnbal, slip);
      logEther('min', min);
      await instance.deposit(usdn, usdnbal, min, {from: member1});
    } catch (e) {
      console.log('deposit usdn', e);
      assert.equal(2, 1, 'deposit usdn failed');
    }
    await logEtherBalance('usdn balance after deposit', USDN, member1);
    ...
    console.log(`
      Withdraw all the tokens from the pool for member1. Convert to USDN.
    `);
    let slip = new BN('100');
    ...
    try {
      await logEtherBalance('usdn balance member1 bef', USDN, member1);
      await instance.withdrawUsd(usdn, bal, minw, { from: member1 });
      await logEtherBalance('usdn balance member1 aft', USDN, member1);
    } catch (e) {
      console.log('withdrawUsd failed', e);
    }
    ...
    console.log(`
      ...
      withdrawal fees. This gets the withdrawal fees from the curveusdn, converts 
