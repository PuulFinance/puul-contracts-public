...
contract("MPH_WETHFarm", async accounts => {
  let instance;
  let helper;
  let factory = '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f';
  let weth = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2';
  let pair = '0x4D96369002fc5b9687ee924d458A7E5bAa5df34E'; // MPH/WETH
  ...
  async function initialize() {
    ...
    instance = await MPH_WETHPool.deployed();
  ...
  it("fund mph/weth Uniswap pool with single sided weth, then usdt", async () => {
    ...
    console.log(`
      Deposit into the MPH/WETH pool using USDT.
    `);
    ...
    console.log('mph_weth reserves', etherFromWei(respair[0]), etherFromWei(respair[1]));

    await logPicoetherBalance('member1 USDT balance', USDT, member1);
    await logEtherBalance('member1 WETH balance', IERC20WETH, member1);
    outs = await helper.estimateOuts([usdt, token, usdt, weth], [dep, dep]);
    ...
    let out = await helper.estimateOut(weth, token, wdep);
    logEther('out', out);
    ...
    let remVox = await logEtherBalance('mph balance after single sided', TOKEN, member1)
    assert.isTrue(remVox.isZero(), 'mph balance should be 0')
    await logEtherBalance('share', instance, member1)

    console.log(`
      Deposit Single Sided from mph
    `);
    await UNI_ROUTER.swapExactTokensForTokens(toWei('2'), 0, [weth, token], member1, Date.now() + 1800, {from: member1});
    const voxbal = await logEtherBalance('mph balance before single sided', TOKEN, member1)

    const vdep = voxbal.div(new BN('2'))

    let outv = await helper.estimateOut(token, weth, vdep);
    logEther('out', outv);
    let tokMinv = slippage(outv, slpSwap);
    logEther('tokMin', tokMinv);

    try {
      logEther('mph balance sending', vdep)
      await instance.depositSingleSided(token, vdep, vdep, tokMinv, slpA, slpB, slpRemA, slpRemB, {from: member1});
      await logEtherBalance('mph balance after single sided', TOKEN, member1)
    } catch (e) {
      console.log('mph deposit failed', e);
      assert.equal(2, 1, 'depositSingleSided failed');
    }
    let voxaft = await logEtherBalance('mph balance after single sided', TOKEN, member1)
    await logEtherBalance('share', instance, member1)

    console.log(`
      Check that the pool balance equals the MPH/WETH pair balance.
   `);
    let pairbal = await PAIR.balanceOf(instance.address);
    let poolbal = await instance.balanceOf(member1);
    logEther('mph_weth balance', pairbal);
    ...
    console.log(`
      Transfer half of the MPH/WETH pool tokens to member2. 
    ...
    console.log(`
      Farm the MPH/WETH farm. Mine 2000 blocks, then harvest the rewards. 
    `);
    ...
    await logEtherBalance('rewardFeesEndpoint mph', TOKEN, rewardFeesEndpoint.address);
    ...
    console.log(`
      Withdraw all the tokens from the MPH/WETH pool for member1. Convert to DAI.
    `);
    ...
    console.log(`
      Withdraw all the tokens from the MPH/WETH pool for member2. Convert to USDT.
    `);
    ...
    console.log(`
      ...
      withdrawal fees. This gets the withdrawal fees from the mph/weth pool, converts 
      to USDC, and adds them to the withdrawal fees endpoint. After this, there 
      should not be anything left in the mph/weth pool.
    `);
    ...
    logEtherBalance('withdrawFeesEndpoint mph', TOKEN, withdrawalFeesEndpoint.address);
    min = [];
    amounts = [];
