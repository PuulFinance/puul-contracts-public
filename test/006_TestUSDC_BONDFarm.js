...
contract("USDC_BONDFarm", async accounts => {
  let instance;
  let helper;
  let factory = '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f';
  let weth = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2';
  let pair = '0x6591c4BcD6D7A1eb4E537DA8B78676C1576Ba244'; // USDC/BOND
  ...
  async function initialize() {
    ...
    instance = await USDC_BONDPool.deployed();
  ...
  it("fund usdc/bond Uniswap pool", async () => {
    ...
    console.log(`
      Deposit into the USDC/BOND pool using USDT.
    `);
    ...
    console.log('usdc_bond reserves', etherFromWei(respair[0]), picoetherFromWei(respair[1]));
    ...
    console.log(`
      Deposit Single Sided from BOND
    `);
    await UNI_ROUTER.swapExactTokensForTokens(toWei('2'), 0, [weth, token], member1, Date.now() + 1800, {from: member1});
    const voxbal = await logEtherBalance('bond balance before single sided', TOKEN, member1)

    const vdep = voxbal.div(new BN('2'))

    let outv = await helper.estimateOut(token, usdc, vdep);
    logPicoether('out', outv);
    let tokMinv = slippage(outv, slpSwap);
    logPicoether('tokMin', tokMinv);

    try {
      logEther('bond balance sending', vdep)
      await instance.depositSingleSided(token, vdep, vdep, tokMinv, slpA, slpB, slpRemA, slpRemB, {from: member1});
      await logEtherBalance('bond balance after single sided', TOKEN, member1)
    } catch (e) {
      console.log('bond deposit failed', e);
      assert.equal(2, 1, 'depositSingleSided failed');
    }
    let voxaft = await logEtherBalance('bond balance after single sided', TOKEN, member1)
    await logEtherBalance('share', instance, member1)

    console.log(`
      Check that the pool balance equals the BOND/USDC pair balance.
   `);
    ...
    console.log(`
      Transfer half of the BOND/USDC pool tokens to member2. 
    ...
    console.log(`
      Time travel one week to get some rewards. 
    `);
    try {
      await instance.earn();
    } catch (e) {
      console.log('harvest failed', e);
      assert.isTrue(1, 2, 'harvest failed');
    }
    ...
    await logEtherBalance('rewardFeesEndpoint BOND', TOKEN, rewardFeesEndpoint.address);
    ...
    logPicoether('out0', outs[0]);

    const minc2 = outs.map(out => out.sub(out.mul(slpr).div(BASE_FEE)));
    minc2.forEach(o => logPicoether('min', o));
    ...
    minc.forEach(o => logPicoether('min', o));
    ...
    console.log(`
      Withdraw all the tokens from the BOND/USDC pool for member1. Convert to DAI.
    `);
    ...
    console.log(`
      The only thing left in the BOND/USDC pool should be the withdrawal fees.
    `);
