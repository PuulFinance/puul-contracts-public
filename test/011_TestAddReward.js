...
contract("Puul Token, Fees", async accounts => {
  ...
  it("PUUL Tests", async () => {
    ...
    `)
    try {
      const POOL = await PICKLE_WETHPool.deployed();
    ...
    console.log(`
      Extract some PUULREW and PUULWTH from the stakingPoolEndpoint and send them to dev
    `)
    const stakingEnd = await PuulStakingPoolEndpoint.deployed();
    const REW = await PuulRewardsFeeToken.deployed();
    const WTH = await PuulWithdrawalFeeToken.deployed();
    ...
    try {
      ...
      const eta = new BN(t.toString()) // seconds 
      const value = new BN('0')
      const sig = 'extractStuckTokens(address,address)'
      const dataRew = encodeParameters(['address', 'address'], [REW.address, developer])
      const dataWth = encodeParameters(['address', 'address'], [WTH.address, developer])
      await timelock12.queueTransaction(stakingEnd.address, value, sig, dataRew, eta)
      await timelock12.queueTransaction(stakingEnd.address, value, sig, dataWth, eta)
      await timeTravel(TIME_1D)
      await logEtherBalance('PUULREW before', REW, developer);
      await logEtherBalance('PUULWTH before', WTH, developer);
      await timelock12.executeTransaction(stakingEnd.address, value, sig, dataRew, eta)
      await timelock12.executeTransaction(stakingEnd.address, value, sig, dataWth, eta)
      await logEtherBalance('PUULREW after ', REW, developer);
      await logEtherBalance('PUULWTH after ', WTH, developer);
    } catch (e) {
      console.log('timelock failed', e)
      assert.isTrue(2, 1, 'timelock failed');
    }
