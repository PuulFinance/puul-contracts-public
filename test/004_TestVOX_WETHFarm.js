...
contract("VOX_WETHFarm", async accounts => {
  let instance;
  let helper;
  let factory = '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f';
  let weth = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2';
  let pair = '0x3D3eE86a2127F4D20b1c533E2c1abd8040da1dd9'; // VOX/WETH
  ...
  async function initialize() {
    ...
    instance = await VOX_WETHPool.deployed();
  ...
  it("fund vox/weth Uniswap pool with single sided weth, then usdt", async () => {
    ...
    console.log('vox_weth reserves', etherFromWei(respair[0]), etherFromWei(respair[1]));
    ...
    let remVox = await logEtherBalance('vox balance after single sided', TOKEN, member1)
    assert.isTrue(remVox.isZero(), 'Vox balance should be 0')
    await logEtherBalance('share', instance, member1)
   ...
   `);
    ...
    logEther('vox_weth balance', pairbal);
