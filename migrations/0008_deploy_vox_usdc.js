...
const VOX_USDCFarm = artifacts.require("VOX_USDCFarm");
...
module.exports = async function (deployer, network, accounts) {
  ...
  deployer.then(async () => {
    ...
    const deployPool = VOX_USDCPool;
    const deployFarm = VOX_USDCFarm;
    ...
    console.log(`VOX_USDCPool: '${pool.address}',`);
    await pool.initialize();
    await pool.setHelper(uhelper.address);
    await pool.setLimits(limits.address);
    // const maxDeposit = web3.utils.toWei('.00007'); // About 5000 USDC
    // const maxTotal = web3.utils.toWei(`${(.00007 * 50).toFixed(6)}`); // About 250K USDC
    ...
    console.log(`VOX_USDCFarm: '${farm.address}',`);
